<template>
  <AppModal @close="closeModal">
    <div class="user-bank">
      ...
      <ValidationObserver class="user-bank__container" tag="div" v-slot="{ invalid, handleSubmit, reset }">
        <form class="user-bank__form" @submit.prevent="handleSubmit(submitUserBank)" @reset.prevent="reset">
          <ValidationProvider
          ...
          <div class="user-bank__btn">
            <button type="submit" :disabled="invalid">{{ $t('ui.button.save') }}</button>
          </div>
        </form>
      </ValidationObserver>
    </div>
